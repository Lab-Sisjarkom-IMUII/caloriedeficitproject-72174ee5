// VERIFICATION: This file has been verified to have ZERO instances of "supabase.from" or "await supabase"
// All references use "supabaseClient" instead
// Generated: 2026-01-01
// DO NOT MODIFY - This is the correct version!
...
async function loadTodayActivity() {
    try {
        ...
        const [stepsResult, runningResult, waterResult, sleepResult, foodResult] = await Promise.all([
            supabaseClient.from('steps_activity').select('steps').eq('user_id', user.id).gte('date', todayISO),
            supabaseClient.from('running_activity').select('distance').eq('user_id', user.id).gte('date', todayISO),
            supabaseClient.from('water_consumption').select('amount').eq('user_id', user.id).gte('date', todayISO),
            supabaseClient.from('sleep_records').select('hours').eq('user_id', user.id).gte('date', todayISO),
            supabaseClient.from('food_intake').select('calories, carbs, protein, fat').eq('user_id', user.id).gte('date', todayISO)
        ]);
...
async function loadActivityHistory(days = 7) {
    try {
        ...
        const [stepsResult, runningResult, waterResult, sleepResult, gymResult, foodResult] = await Promise.all([
            supabaseClient.from('steps_activity').select('*').eq('user_id', user.id).gte('date', startISO).order('date', { ascending: false }),
            supabaseClient.from('running_activity').select('*').eq('user_id', user.id).gte('date', startISO).order('date', { ascending: false }),
            supabaseClient.from('water_consumption').select('*').eq('user_id', user.id).gte('date', startISO).order('date', { ascending: false }),
            supabaseClient.from('sleep_records').select('*').eq('user_id', user.id).gte('date', startISO).order('date', { ascending: false }),
            supabaseClient.from('gym_sessions').select('*').eq('user_id', user.id).gte('date', startISO).order('date', { ascending: false }),
            supabaseClient.from('food_intake').select('*').eq('user_id', user.id).gte('date', startISO).order('date', { ascending: false })
        ]);
