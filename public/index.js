...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    function initializeGoogleSignIn() {
        ...
        if (typeof google !== 'undefined' && google.accounts && google.accounts.id) {
            try {
                google.accounts.id.initialize({
                    client_id: '329300608269-tpv1dff78acj6j0avi07872u0kvgkhn1.apps.googleusercontent.com',
                    callback: handleGoogleSignIn,
                    auto_select: false,
                    cancel_on_tap_outside: true
                });
                ...
                if (buttonElement) {
                    ...
                    console.warn('⚠️ g_id_signin element not found');
                }
            } catch (err) {
                console.error('❌ Google Sign-In error:', err);
            }
        } else {
            console.warn('⚠️ Google API not ready, retrying in 1s...');
...
// ========================================
// FIX GOOGLE BUTTON RENDER
// Paste di paling bawah public/index.js
// Replace forceRenderGoogleButton function yang lama
// ========================================

function forceRenderGoogleButton() {
    console.log('🔵 Force rendering Google button (CENTERED)...');
    
    if (typeof google === 'undefined' || !google.accounts || !google.accounts.id) {
        console.warn('⚠️ Google API not ready, retry in 1s...');
        setTimeout(forceRenderGoogleButton, 1000);
        return;
    }
    ...
    try {
        ...
        // Render with FULL WIDTH and CENTERED
        google.accounts.id.renderButton(btn, {
            ...
            text: 'signin_with',
            size: 'large',
            width: btn.offsetWidth || 400,  // Use parent width!
            logo_alignment: 'left'
        });
        
        console.log('✅ Google button rendered (width:', btn.offsetWidth, ')');
...
// Render on page load
setTimeout(forceRenderGoogleButton, 2000);

// Re-render when modal opens (to get correct width)
document.addEventListener('click', function(e) {
    if (e.target.closest('[onclick*="toggleForm"]') || 
        e.target.textContent.includes('Masuk')) {
        // Wait for modal to fully render
        setTimeout(() => {
            const btn = document.querySelector('.g_id_signin');
            if (btn && btn.offsetWidth > 0) {
                console.log('🔄 Re-rendering button with width:', btn.offsetWidth);
                forceRenderGoogleButton();
            }
        }, 300);
    }
});